...
import com.castelcode.travelcompanion.agenda_entry.DateEntryActivity;
import com.castelcode.travelcompanion.agenda_entry.DateString;
import com.castelcode.travelcompanion.R;
import com.castelcode.travelcompanion.adapters.ExpandableListAdapter;
...
import com.castelcode.travelcompanion.utils.TreeMapConverter;
import com.castelcode.travelcompanion.utils.TripAgendaConstants;
...
public class TripAgenda extends AppCompatActivity implements View.OnClickListener,
    ...
    protected void onPause() {
        ...
        ref.child(TripAgendaConstants.USER_PATH).child(Build.MODEL).removeValue();
    }
    ...
    private void resetConnectionFor(String sessionId) {
        ref.child(sessionId).removeValue((DatabaseError databaseError, DatabaseReference databaseReference) -> {
            if(databaseError != null) {
                ...
                ref.child(TripAgendaConstants.USER_PATH).child(Build.MODEL)
    ...
    private void addItemsShared(String readMessage) {
        ...
        for (String objectsAsString : objectsAsStrings) {
            if (!objectsAsString.contains("|")) {
                ...
                if (parts.length > 0) {
                    String title = parts[0]; // The title is required when creating an agenda item.
                    String time = parts.length > 1 ? parts[1] : "";
                    String location = parts.length > 2 ? parts[2] : "";
                    String description = parts.length > 3 ? parts[3] : "";

                    DateEntry dateEntry =
                            new DateEntry(title, time, location, description, date);
    ...
    private void handleDataTransfer(DataSnapshot snapshot) {
        String val;
        if (endDate == null) {
            Toast.makeText(this, "You must set an end date before receiving shared data.",
                    Toast.LENGTH_SHORT).show();
            return;
        }
